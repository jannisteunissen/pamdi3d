...
contains
   ...
   recursive subroutine compute_potential_recursive(amr_grid, time, zero_bc)
      ...
      if (present(zero_bc)) then
         use_homogenuous_dirichlet = zero_bc
      else
   ...
   recursive subroutine set_new_grids_recursive(amr_grid)
      ...
      if (n_ref > 0) then
         ...
         do nc = 1, n_child
            ...
            ! Mark child region
            amr_grid%child_ix(i_min(1):i_max(1), i_min(2):i_max(2), i_min(3):i_max(3)) = nc

            ! Create child, has 2 * (i_max-i_min) + 1 points
            call set_grid(amr_grid%children(nc), amr_grid%lvl+1, 2 * (i_max-i_min) + 1, &
                 E_ix_to_xyz(amr_grid, i_min), E_ix_to_xyz(amr_grid, i_max), parent=amr_grid)
            call set_new_grids_recursive(amr_grid%children(nc))
         end do
      end if
   end subroutine set_new_grids_recursive

   ! Given a list of points (point_list), returns a list of boxes (out_boxes) that enclose all the
   ! points. The squared distance between these boxes is at least min_dist2. The boxes furthermore
   ! are placed between i_min and i_max, have a buffer_width around the points they enclose and have
   ! minimal size min_size.
   subroutine get_boxes(point_list, out_boxes, min_dist2, i_min, i_max, buffer_width, min_size)
      ...
      do
         ...
         do ix = 1, n_boxes
            ...
            do nn = n_boxes_new, 1, -1 ! Reverse order, since latest new box is probably closer
         ...
         do j = 1, Ny
            do i = 1, Nx
               ...
               if (temp <= elec_radius) then
                  ...
                  amr_grid%vars(i,j,1, E_i_pot) = voltage * max(0.0_dp, 1 - (temp-elec_radius)/decay_range)
               end if
            end do
         end do
   ...
   subroutine set_bc_from_parent(amr_grid, v_ixs)
      ...
      do k = 1, Nz, Nz-1
         do j = 1, Ny
            do i = 1, Nx
               xyz = E_ix_to_xyz(amr_grid, (/i, j, k/))
               amr_grid%vars(i,j,k, v_ixs) = get_vars_at_grid(amr_grid%parent, v_ixs, xyz)
            end do
         end do
      end do
   ...
   integer function get_child_ix_at_xyz(amr_grid, xyz)
      ...
      get_child_ix_at_xyz = minval(amr_grid%child_ix(ixs(1):ixs(1)+1, ixs(2):ixs(2)+1, ixs(3):ixs(3)+1))
   ...
   integer function get_child_ix_at_ixs(amr_grid, ixs)
      ...
      get_child_ix_at_ixs = minval(amr_grid%child_ix(ixs(1):ixs(1)+1, ixs(2):ixs(2)+1, ixs(3):ixs(3)+1))
   ...
   subroutine E_get_max_of_vars(v_ixs, max_values)
      ...
      do gg = 1, size(grid_list)
         do vv = 1, size(v_ixs)
            max_values(vv) = max(max_values(vv), maxval(grid_list(gg)%ptr%vars(:,:,:,v_ixs(vv))))
         end do
      end do
   ...
   subroutine allocate_grid_data(amr_grid)
      type(amr_grid_t), intent(inout)  :: amr_grid
      allocate(amr_grid%vars(amr_grid%Nr(1), amr_grid%Nr(2), amr_grid%Nr(3), E_n_vars))
      allocate(amr_grid%child_ix(amr_grid%Nr(1), amr_grid%Nr(2), amr_grid%Nr(3)))
